              ...
              <Dialog.Panel className='relative flex w-full max-w-xs flex-col overflow-y-auto bg-white pb-12 shadow-xl'>
                ...
                <Tab.Group as='div' className='mt-2'>
                  ...
                  <Tab.Panels as={Fragment}>
                    {navigation.navbar.map((element) => (
                      <Tab.Panel key={element.name} className='space-y-10 px-4 pb-8'>
                        {element.items.map((item) => (
                          <ul role='list' aria-labelledby={`${element.id}-${item.id}-heading-mobile`} className='mt-6 flex flex-col space-y-6'>
                            <li key={item.name} className='flow-root'>
                              <a href={item.href} className='-m-2 ml-0 block text-gray-500'>
                                {item.name}
                              </a>
                            </li>
                          </ul>
                ...
                <div className='border-t border-gray-200 py-6 px-4'>
                  <a href='#' className='-m-2 flex items-center p-2'>
                    ...
                    />
                    <span className='ml-3 block text-base font-medium text-gray-900'>VIE</span>
            ...
            <div className='flex h-16 items-center justify-between'>
              ...
              <Popover.Group className='hidden lg:ml-8 lg:block lg:self-stretch'>
                <div className='flex h-full space-x-8'>
                  {navigation.navbar.map((element) => (
                    <Popover key={element.name} className='flex'>
                      {({ open }) => (
                        <>
                          ...
                          <Transition
                            ...
                            <Popover.Panel className='absolute top-full text-sm text-gray-500'>
                              ...
                              <div className='absolute top-1/2 bg-white shadow' aria-hidden='true' />

                              <div className='relative bg-white'>
                                <div className='mx-auto max-w-7xl'>
                                  <div className='border rounded-md p-2 min-w-[150px]'>
                                    <div className='row-start-1 grid grid-cols-1 text-sm'>
                                      {element.items.map((item) => (
                                        <div key={item.name}>
                                          <ul role='list' aria-labelledby={`${item.name}-heading`} className='mt-6 space-y-6 sm:mt-4 sm:space-y-4'>
